...
h = rcosdesign(alpha, 8, Ns); %Reponse impulsionnelle de racine de cosinus sur ́elev ́e
filt = filter(h, 1, map); 
...
ylabel('map(t)');
title('Mapping ');
saveas(fig, "figures/mod4_NRZ.png");
...
saveas(fig, "figures/mod4_signal.png");

%% DSP theorique 

Sx_theo = @(f) (1/Ts)*(Ts*(abs(f)<=( (1-alpha)/(2*Ts) )) + (Ts/2)*(1+cos( ((pi*Ts)/alpha)*(abs(f) - (1-alpha)/(2*Ts)) )).*( (1-alpha)/(2*Ts)<=abs(f) & abs(f)<=(1+alpha)/(2*Ts) )) ;
...
hold off;
saveas(fig, "figures/mod4_DSP1.png");
...
hold off;
legend("DSP", "DSP Théorique");
saveas(fig, "figures/mod4_DSP2.png");