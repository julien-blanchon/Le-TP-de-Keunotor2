...
for i=1:length(EbN0)
    ...
    %% Filtre reception1
    hr1 = ones(1, Ns); %Reponse impulsionnelle de type rectangulaire de duree Ts
    x1r = filter(hr1, 1, x1c);
    X1 = [X1; x1r];
    
    %% Filtre reception2
    hr2 = [ones(1, Ns/2) zeros(1, Ns/2)]; %Reponse impulsionnelle de type rectangulaire de duree Ts/2
    ...
    SymbolesDecides2 = sign(z2);
    y2 = (SymbolesDecides2+1)/2;
...
ylabel("TEB");
saveas(fig, "figures/ComparaisonChaineRefEt1TEB.png");
...
set(gca,'yscale','log');
hold off
legend('TES simulé chaine de référence','TES simulé chaine 1');
title(sprintf("Taux erreur symbole TES = f(E_b/N_0) en dB"));
xlabel("E_b/N_0 en dB");
ylabel("TES");
saveas(fig, "figures/ComparaisonChaineRefEt1TES.png");