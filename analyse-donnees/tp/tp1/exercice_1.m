...
Vn = 2;
Bn = 3;
r_RV = sqrt(Sigma(Rn, Vn)/(Sigma(Rn,Rn)*Sigma(Vn,Vn))) %0.0320 1er
r_RB = sqrt(Sigma(Rn, Bn)/(Sigma(Rn,Rn)*Sigma(Bn,Bn))) %0.0221 3e
r_VB = sqrt(Sigma(Vn, Bn)/(Sigma(Vn,Vn)*Sigma(Bn,Bn))) %0.0237 2e



% Proportions de contraste : Beaucoup de variance relativement a la
% variance globale
c_R = Sigma(Rn,Rn)/(Sigma(Rn,Rn)+Sigma(Vn,Vn)+Sigma(Bn,Bn)) %0.1962 2e
c_V = Sigma(Vn,Vn)/(Sigma(Rn,Rn)+Sigma(Vn,Vn)+Sigma(Bn,Bn)) %0.1907 3e
c_B = Sigma(Bn,Bn)/(Sigma(Rn,Rn)+Sigma(Vn,Vn)+Sigma(Bn,Bn)) %0.6131 1er 
